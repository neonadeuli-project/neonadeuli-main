...
def get_pr_content(repo, pr_number, token):
    ...
    try:
        ...
        logger.error(f"PR 내용 가져오기 오류: {str(e)}")
        return None
...
def post_pr_comment(repo, pr_number, body, token):
    ...
    try:
        response = requests.post(url, headers=headers, json=data)
        response.raise_for_status()
        logger.info("성공적으로 PR 코멘트가 게시되었습니다.")
    except requests.RequestException as e:
...
def main():
    try:
        ...
        pr_content = get_pr_content(repo, pr_number, github_token)
        ...
        if pr_content:
            review = review_code(pr_content)
            comment_body = f"AI Code Review:\n\n{review}"
            post_pr_comment(repo, pr_number, comment_body, github_token)
        else:
            logger.warning("PR 내용을 가져오는 데 실패했습니다.")
